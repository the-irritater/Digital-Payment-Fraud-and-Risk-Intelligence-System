...
def test_causal_customer_mean():
    ...
    c100_first = featured[(featured["nameOrig"] == "C100") & (featured["step"] == 1)]
...
def test_new_beneficiary_flag():
    ...
    assert (
        c100_to_c300.iloc[0]["is_new_beneficiary"] == 1
    ), "First occurrence should be new"
    assert (
        c100_to_c300.iloc[1]["is_new_beneficiary"] == 0
    ), "Second occurrence should NOT be new"
...
def test_log_amount_positive():
    ...
    featured = build_features(df)
    assert (
        featured["log_amount"] > 0
    ).all(), "log_amount should be > 0 for positive amounts"
...
def test_night_time_flag():
    ...
    for _, row in featured.iterrows():
        ...
        assert (
            row["is_night_time"] == expected_night
        ), f"Night flag wrong for hour {hour}"
...
def test_temporal_leakage_prevention():
    ...
    for step in [1, 2, 3, 10, 11]:
        orig_row = featured_original[featured_original["step"] == step][
            earlier_cols
        ].iloc[0]
        mod_row = featured_modified[featured_modified["step"] == step][
            earlier_cols
        ].iloc[0]
        diff = (orig_row - mod_row).abs().sum()
        assert (
            diff < 1e-10
        ), f"Temporal leakage detected at step {step}: features changed when future transaction was modified"
