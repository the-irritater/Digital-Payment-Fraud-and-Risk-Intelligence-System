...
def calculate_psi(
    baseline: np.ndarray, current: np.ndarray, num_bins: int = 10
) -> float:
    """
    ...
    eps = 1e-4
    baseline_pct = (baseline_counts + eps) / (
        len(baseline) + eps * len(baseline_counts)
    )
...
class DriftDetector:
    def __init__(
        self, baseline_df: pd.DataFrame = None, feature_cols: List[str] = None
    ):
        self.baseline_df = baseline_df
        self.feature_cols = (
            feature_cols
            if feature_cols
            else list(baseline_df.columns) if baseline_df is not None else []
        )
        self.baseline_stats = {}
    ...
    def detect_feature_drift(self, current_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        for col in self.feature_cols:
            ...
            ks_stat, p_val = (
                ks_2samp(base_vals, curr_vals) if len(curr_vals) > 0 else (0.0, 1.0)
            )
            ...
            results[col] = {
                ...
                "status": status,
                "current_mean": (
                    round(float(np.mean(curr_vals)), 4) if len(curr_vals) > 0 else 0.0
                ),
        ...
        overall_status = (
            ...
            else "WARNING" if max_psi >= 0.10 else "HEALTHY"
        )
...
if __name__ == "__main__":
    ...
    base_data = pd.DataFrame(
        {
            "amount": np.random.normal(5000, 1000, 1000),
            "velocity": np.random.poisson(3, 1000),
        }
    )
    shifted_data = pd.DataFrame(
        {
            "amount": np.random.normal(15000, 5000, 500),
            "velocity": np.random.poisson(8, 500),
        }
    )
