...
class BusinessKPITracker:
    ...
    def calculate_business_kpis(
        ...
        total_fraud_tx = (
            int(df_evaluations[actual_fraud_col].sum())
        ...
        total_fraud_volume = (
            float(
                df_evaluations[df_evaluations[actual_fraud_col] == 1][amount_col].sum()
            )
            if actual_fraud_col in df_evaluations.columns
            else 0.0
        )
        ...
        if actual_fraud_col in df_evaluations.columns:
            ...
            cost_per_detected_fraud = total_financial_loss / max(
                len(captured_fraud_tx), 1
            )
        else:
