...
def temporal_train_val_test_split(
    ...
    val_df = df_sorted[(df_sorted['step'] > train_step_cutoff) & (df_sorted['step'] <= val_step_cutoff)].copy()
    test_df = df_sorted[df_sorted['step'] > val_step_cutoff].copy()
    
    print(f"Temporal Split Results:")
    print(f"  Train Set: {len(train_df):,} rows (Steps {train_df['step'].min()}-{train_df['step'].max()}) | Fraud: {train_df['isFraud'].sum():,} ({train_df['isFraud'].mean()*100:.3f}%)")
    print(f"  Val Set:   {len(val_df):,} rows (Steps {val_df['step'].min()}-{val_df['step'].max()}) | Fraud: {val_df['isFraud'].sum():,} ({val_df['isFraud'].mean()*100:.3f}%)")
    print(f"  Test Set:  {len(test_df):,} rows (Steps {test_df['step'].min()}-{test_df['step'].max()}) | Fraud: {test_df['isFraud'].sum():,} ({test_df['isFraud'].mean()*100:.3f}%)")
...
if __name__ == "__main__":
    ...
    for row in summary['fraud_by_type']:
        print(f"  {row['type']:10s}: {row['total_transactions']:10,} txs | Fraud: {row['fraud_count']:6,} | Rate: {row['fraud_rate_pct']:.4f}%")
