...
def generate_synthetic_upi_dataset(
    num_records: int = 25000, fraud_rate: float = 0.02, seed: int = 42
) -> pd.DataFrame:
    """
    ...
    np.random.seed(seed)
    print(
        f"Generating {num_records:,} synthetic Indian UPI transactions (Fraud rate target: ~{fraud_rate*100}%)..."
    )
    ...
    for i in range(num_records):
        ...
        amount = float(
            np.round(
                max(
                    np.random.normal(persona["amount_mean"], persona["amount_std"]),
                    10.0,
                ),
                2,
            )
        ...
        is_new_beneficiary = (
            1 if random.random() < persona["new_beneficiary_prob"] else 0
        )
        ...
        daily_rate = max(
            int(
                np.random.normal(persona["tx_per_day_mean"], persona["tx_per_day_std"])
            ),
            1,
        )
        ...
        avg_amount_30d = float(
            np.round(
                np.random.normal(
                    persona["amount_mean"] * 0.9, persona["amount_std"] * 0.3
                ),
                2,
            )
        )
    ...
    print(
        f"Shape: {df_upi.shape} | Fraud cases: {df_upi['is_fraud'].sum():,} ({df_upi['is_fraud'].mean()*100:.2f}%)"
    )
