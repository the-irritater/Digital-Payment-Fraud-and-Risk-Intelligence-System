...
def _compute_rolling_features_for_group(group: pd.DataFrame) -> pd.DataFrame:
    ...
    prior_mean = np.where(
        prior_counts > 0, cum_prior_amounts / np.maximum(prior_counts, 1), amounts
    )
...
def build_features(df: pd.DataFrame, is_training: bool = True) -> pd.DataFrame:
    ...
    data["orig_balance_err"] = (data["oldbalanceOrg"] - data["amount"]) - data[
        "newbalanceOrig"
    ]
    data["dest_balance_err"] = (data["oldbalanceDest"] + data["amount"]) - data[
        "newbalanceDest"
    ]
...
def get_feature_schema() -> Dict[str, Any]:
    ...
    return {
        ...
        "features": {
            ...
            "hour_of_day": {
                "type": "categorical",
                "description": "Hour extracted from step (step % 24)",
                "leakage_risk": "none",
            },
            "day_of_week": {
                "type": "categorical",
                "description": "Day of week from step",
                "leakage_risk": "none",
            },
            "is_night_time": {
                "type": "binary",
                "description": "1 if hour in [0,1,2,3,4,5]",
                "leakage_risk": "none",
            },
            "type_TRANSFER": {
                "type": "binary",
                "description": "Transaction type one-hot",
                "leakage_risk": "none",
            },
            "type_CASH_OUT": {
                "type": "binary",
            ...
            "is_orig_customer": {
                "type": "binary",
                "description": "Originator is customer account (C prefix)",
                "leakage_risk": "none",
            },
            "is_dest_customer": {
                "type": "binary",
                "description": "Destination is customer account",
                "leakage_risk": "none",
            },
            "is_dest_merchant": {
                "type": "binary",
                "description": "Destination is merchant account (M prefix)",
                "leakage_risk": "none",
            },
            "orig_balance_err": {
                "type": "continuous",
                "description": "Discrepancy: (old_bal - amount) - new_bal",
                "leakage_risk": "PaySim artifact",
            },
            "dest_balance_err": {
                "type": "continuous",
                "description": "Discrepancy: (old_bal + amount) - new_bal",
            ...
            "is_zero_orig_balance": {
                "type": "binary",
                "description": "Originator balance is zero before tx",
                "leakage_risk": "none",
            },
            "is_zero_dest_balance": {
                "type": "binary",
                "description": "Destination balance is zero before tx",
                "leakage_risk": "none",
            },
            "amount_to_orig_prior_mean_ratio": {
                "type": "continuous",
                "description": "Current amount / expanding prior mean for customer",
                "leakage_risk": "mitigated (causal expanding window)",
            },
            "transactions_last_1h": {
                "type": "count",
                "description": "Rolling count of customer transactions in prior 1 hour",
                "leakage_risk": "mitigated (backward-looking searchsorted)",
            },
            "transactions_last_6h": {
                "type": "count",
                "description": "Rolling count of customer transactions in prior 6 hours",
                "leakage_risk": "mitigated (backward-looking searchsorted)",
            },
            "transactions_last_24h": {
                "type": "count",
                "description": "Rolling count of customer transactions in prior 24 hours",
                "leakage_risk": "mitigated (backward-looking searchsorted)",
            },
            "is_new_beneficiary": {
                "type": "binary",
                "description": "First occurrence of (originator, destination) pair",
                "leakage_risk": "mitigated (cumcount)",
            },
            "amount_velocity_6h": {
                "type": "continuous",
                "description": "Total amount transferred by customer in prior 6 hours",
                "leakage_risk": "mitigated (backward-looking cumsum)",
            },
        },
    }
