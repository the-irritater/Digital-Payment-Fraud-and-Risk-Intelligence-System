...
class CustomerStateStore:
    ...
    def seed_from_dataframe(self, df: pd.DataFrame):
        ...
        print(f"[CustomerStateStore] Seeding customer state from {len(df):,} historical transactions...")
    ...
    def compute_realtime_features(self, tx_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        amt_vel_6h = sum(a for s, a in zip(steps_history, amounts_history) if (step - 6) <= s <= step) + amt
