...
def build_threshold_cost_table(
    ...
    for thresh in thresholds:
        metrics = evaluate_predictions(
            y_true, y_prob, amounts, threshold=thresh, fp_cost=fp_cost
        )
        rows.append(metrics)
...
def run_ablation_experiment(
    ...
    model_no_bal.fit(
        X_tr_no_bal, y_train, eval_set=[(X_val_no_bal, y_val)], verbose=False
    )
    probs_no_bal = model_no_bal.predict_proba(X_te_no_bal)[:, 1]
    metrics_no_bal = evaluate_predictions(
        y_test, probs_no_bal, test_amounts, threshold=0.5
    )
...
def _run_weight_optimization(
    ...
    for weights in candidate_weights:
        ...
        composite_prob = (
            ...
            * weights[
                "w_rules"
            ]  # rules contribution approximated as 0 for optimization
        )
        # Normalize to [0, 1] for threshold evaluation
        composite_prob = np.clip(composite_prob, 0.0, 1.0)

        _, _, opt_metrics = build_threshold_cost_table(
            y_val, composite_prob, val_amounts, fp_cost
        )
    ...
    print(
        f"Best weights: ML={best_weights['w_ml']}, Anomaly={best_weights['w_anomaly']}, Rules={best_weights['w_rules']}"
    )
...
def train_and_evaluate_all(
    sample_size: int = 400000, n_optuna_trials: int = 15
) -> Dict[str, Any]:
    ...
    if sample_size and len(df_filtered) > sample_size:
        print(
            f"Sampling {sample_size:,} records for fast, high-performance training..."
        )
        df_filtered = (
            df_filtered.sample(n=sample_size, random_state=42)
            .sort_values("step")
    ...
    train_df, val_df, test_df = temporal_train_val_test_split(
        featured_df, train_ratio=0.70, val_ratio=0.15
    )
    ...
    results["Logistic_Regression"] = evaluate_predictions(
        y_val, lr_val_prob, val_amounts, threshold=0.5
    )
    ...
    results["Random_Forest"] = evaluate_predictions(
        y_val, rf_val_prob, val_amounts, threshold=0.5
    )
    ...
    calibrated_xgb = CalibratedClassifierCV(
        estimator=base_xgb, method="sigmoid", cv="prefit"
    )
    ...
    print(
        f"LOCKED Optimal Threshold selected on Validation Set: {locked_threshold:.4f}"
    )
    ...
    print(
        f"PR-AUC: {final_test_metrics['pr_auc']:.4f} | ROC-AUC: {final_test_metrics['roc_auc']:.4f}"
    )
    print(
        f"Precision: {final_test_metrics['precision']:.4f} | Recall: {final_test_metrics['recall']:.4f} | F1: {final_test_metrics['f1_score']:.4f}"
    )
    print(
        f"Brier Score: {final_test_metrics['brier_score']:.6f} | ECE: {final_test_metrics['expected_calibration_error']:.6f}"
    )
    print(
        f"Missed Fraud Loss: ₹{final_test_metrics['missed_fraud_loss_inr']:,.2f} | Investigation Cost: ₹{final_test_metrics['investigation_cost_inr']:,.2f}"
    )
    print(
        f"Total Expected Financial Loss: ₹{final_test_metrics['total_financial_loss_inr']:,.2f}"
    )
    ...
    metadata = {
        ...
        "calibration": {
            ...
            "expected_calibration_error": final_test_metrics[
        ...
        "training_duration_seconds": round(
            (training_end - training_start).total_seconds(), 1
        ),
    }
