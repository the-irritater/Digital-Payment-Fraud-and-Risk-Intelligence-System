...
class RiskEngine:
    def __init__(
        self, w_ml: float = None, w_anomaly: float = None, w_rules: float = None
    ):
        """
        ...
        self.w_anomaly = (
            w_anomaly
            if w_anomaly is not None
            else default_weights.get("w_anomaly", 0.20)
        )
        self.w_rules = (
            w_rules if w_rules is not None else default_weights.get("w_rules", 0.20)
        )
    ...
    def _load_anomaly_model(self):
        ...
        if os.path.exists(ISO_FOREST_PATH):
            ...
            print(
                f"[RiskEngine] Isolation Forest not found at {ISO_FOREST_PATH}. "
        ...
        try:
            if hasattr(self.iso_forest, "feature_names_in_"):
                full_row = {
                    ...
                    for col in self.iso_forest.feature_names_in_
                }
    ...
    def evaluate_business_rules(
        self, tx: Dict[str, Any], feature_row: pd.Series
    ...
    def calculate_risk(
        self, ml_prob: float, feature_row: pd.Series, tx_dict: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        final_risk_score = min(
            max(ml_component + anomaly_component + rules_component, 0.0), 100.0
        )
...
if __name__ == "__main__":
    ...
    result = engine.calculate_risk(
        ml_prob=0.945, feature_row=dummy_feature_row, tx_dict=dummy_tx
    )
