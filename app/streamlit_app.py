...
def get_services():
    ...
    threshold_csv = os.path.join(
        os.path.dirname(__file__), "..", "reports", "threshold_cost_table.csv"
    )
    df_thresh = (
        pd.read_csv(threshold_csv) if os.path.exists(threshold_csv) else pd.DataFrame()
    )

    # Load dataset summary if available
    summary_path = os.path.join(
        os.path.dirname(__file__), "..", "reports", "dataset_summary.json"
    )
    dataset_summary = {}
...
st.sidebar.info(
    f"Risk Engine: ONLINE ({risk_engine.w_ml:.0%} ML / {risk_engine.w_anomaly:.0%} Anomaly / {risk_engine.w_rules:.0%} Rules)"
)
...
if nav_option == "Executive Overview":
    ...
    st.markdown(
        "Model performance metrics, dataset provenance, and financial loss indicators — all loaded from pipeline artifacts."
    )
    ...
    total_records = dataset_summary.get(
        "total_records", meta.get("data_provenance", {}).get("sample_size", "N/A")
    )
    ...
    with c2:
        ...
        kpi_data = pd.DataFrame(
            [
                ...
                {
                    "KPI": "Blocked Fraud Value (INR)",
                    "Value": f"₹{blocked_value_val:,.0f}",
                },
...
# TAB 2: TRANSACTION RISK CHECKER
    ...
    st.markdown(
        "Simulate single digital payment transactions with **stateful customer baselines** and inspect Risk Engine decisions."
    )
    ...
    with p_col3:
        if st.button(
            "Preset 3: New Beneficiary Velocity Surge", use_container_width=True
        ):
            scenario = "surge"
    ...
    if scenario == "legit":
        (
            ...
            default_orig,
            default_dest,
        ) = (1250.0, "PAYMENT", 14, 15000.0, "C928310482", "M102938471")
        ...
        (
            ...
            default_orig,
            default_dest,
        ) = (84500.0, "TRANSFER", 2, 84500.0, "C928310482", "C938201941")
    elif scenario == "surge":
        (
            default_amt,
            default_type,
            ...
            default_orig,
            default_dest,
        ) = (45000.0, "CASH_OUT", 1, 45000.0, "C482019482", "C849201942")
    else:
        (
            default_amt,
            default_type,
            ...
            default_orig,
            default_dest,
        ) = (25000.0, "TRANSFER", 3, 25000.0, "C928310482", "C102938471")
    ...
    with st.form("tx_input_form"):
        ...
        with c1:
            ...
            hour_of_day = st.slider(
                "Hour of Day (00:00 - 23:00)", 0, 23, value=default_hour
            )
        with c2:
            old_bal_org = st.number_input(
                "Originator Balance Before Tx (INR)",
                min_value=0.0,
                value=default_old_bal,
            )
            new_bal_org = st.number_input(
                "Originator Balance After Tx (INR)",
                min_value=0.0,
                value=(
                    0.0
                    if default_old_bal == amount
                    else max(default_old_bal - amount, 0.0)
                ),
            )
        with c3:
        ...
        submit_btn = st.form_submit_button(
            "Evaluate Transaction Risk", use_container_width=True
        )
    ...
    if submit_btn or scenario is not None:
        ...
        anomaly_status = (
            "ACTIVE"
            if risk_result["components"].get("anomaly_model_active", False)
            else "BASELINE"
        )
        ...
        comp_df = pd.DataFrame(
            [
                {
                    "Component": f"Calibrated XGBoost ML",
                    "Points": risk_result["components"]["ml_contribution"],
                },
                {
                    "Component": f"Isolation Forest Anomaly [{anomaly_status}]",
                    "Points": risk_result["components"]["anomaly_contribution"],
                },
                {
                    "Component": "Business Rules Engine",
                    "Points": risk_result["components"]["rule_contribution"],
                },
            ]
        )
        fig_comp = px.bar(
            ...
            y="Points",
            color="Component",
            title="Contribution to 0-100 Composite Risk Score",
        )
        ...
        if risk_result["triggered_rules"]:
            for rule in risk_result["triggered_rules"]:
                st.warning(
                    f"**[{rule['rule_id']}] {rule['severity']} SEVERITY**: {rule['description']}"
                )
        else:
...
# TAB 3: FRAUD INVESTIGATION QUEUE
    ...
    st.markdown(
        "Decision queue connected to the **SQLite Warehouse (`data/processed/fraud_intelligence.db`)**."
    )
    ...
    with qc2:
        new_status = st.selectbox(
            ...
            ["CONFIRMED_FRAUD", "FALSE_POSITIVE", "UNDER_REVIEW", "ALLOW"],
        )
    with qc3:
    ...
    if st.button("Submit Case Decision"):
        ...
        st.success(
            f"Case {sel_case} successfully updated to '{new_status}' in SQLite Database by {analyst_id}."
        )
...
# TAB 4: MODEL AND RISK ANALYTICS
    ...
    st.markdown(
        "Performance benchmarks, calibration metrics, and **actual validation threshold-cost curves** loaded from pipeline artifacts."
    )
    ...
    col3.metric(
        "Recall @ Locked Thresh",
        f"{recall_val*100:.1f}%",
        f"{precision_val*100:.1f}% Precision",
    )
    ...
    with c1:
        ...
        if (
            not df_thresh.empty
            and "recall" in df_thresh.columns
            and "precision" in df_thresh.columns
        ):
            fig_pr = px.line(
                df_thresh.sort_values("recall"),
                x="recall",
                y="precision",
            ...
            st.warning(
                "Threshold cost table not found. Run `python src/train.py` to generate."
            )
    ...
    with c2:
        ...
        if (
            not df_thresh.empty
            and "threshold" in df_thresh.columns
            and "total_financial_loss_inr" in df_thresh.columns
        ):
            fig_loss = px.line(
                df_thresh,
                x="threshold",
                y="total_financial_loss_inr",
            ...
            fig_loss.add_vline(
                ...
                line_dash="dash",
                line_color="#ef4444",
                annotation_text=f"Locked {opt_thresh:.2f}",
            )
    ...
    if cal_info:
        ...
        cal_c3.metric(
            ...
            f"{cal_info.get('expected_calibration_error', 0):.6f}",
        )
...
# TAB 5: SYNTHETIC UPI AND GRAPH ANALYTICS
    ...
    st.markdown(
        "Network MultiDiGraph visualization for money mule rings, circular payment chains, and synthetic UPI app metadata."
    )

    upi_path = os.path.join(
    ...
    with gc1:
        fig_app = px.pie(
            df_upi,
            names="upi_app",
            title="UPI App Market Share Distribution",
    ...
    with gc2:
        fig_cat = px.bar(
            df_upi.groupby("merchant_category")["is_fraud"]
            .agg(["count", "sum"])
            .reset_index(),
            x="merchant_category",
            y="sum",
            text="sum",
            title="Fraud Occurrences by Merchant Category",
            color="sum",
